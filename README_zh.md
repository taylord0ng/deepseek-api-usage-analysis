...
- **总览** — KPI 卡片（费用、Token、缓存命中率、活跃 Key）+ 每日费用柱状图 + 各 Key 费用环形图
- **按 Key** — 可排序的表格，展示每个 Key 的 Token 数、费用、缓存命中率、请求次数及内嵌用量条
- **缓存** — 缓存命中率仪表盘、每日缓存趋势图、各 Key 缓存命中/未命中堆叠图
...
Next.js 16（静态导出）· React 19 · TypeScript · ECharts · Papa Parse · Tailwind CSS 4
