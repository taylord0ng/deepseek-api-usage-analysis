...
**Version**: 0.6.6
- **New in this version**: Homepage + landing page SEO metadata optimized for cost tracker keywords, explicit keyword-rich subtitles added below H1 on homepage and all three tool landing pages.
...
│   ├── ChangelogPage.tsx      # Changelog page: complete version history (v0.1.0–v0.6.6), entries by category with color-coded dots, JSON-LD WebPage schema, bilingual, Apple-minimalist layout
...
- **`/changelog`** (`ChangelogPage.tsx`): Complete version history from v0.1.0 to v0.6.6, entries organized by category (Added/Improved/Fixed/Dependencies) with color-coded dots, JSON-LD WebPage schema, bilingual, Apple-minimalist layout matching privacy/terms pages. Priority 0.5 in sitemap.
