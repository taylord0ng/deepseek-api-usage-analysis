...
const METRICS: { key: MetricKey; label: string; color: string; format: (v: number) => string }[] = [
  { key: "cost", label: "Daily Cost", color: "#3b82f6", format: (v) => `¥${v.toFixed(2)}` },
  { key: "tokens", label: "Daily Tokens", color: "#8b5cf6", format: (v) => (v >= 1_000_000 ? `${(v / 1_000_000).toFixed(1)}M` : v >= 1000 ? `${(v / 1000).toFixed(0)}K` : String(v)) },
  { key: "cacheHitRate", label: "Cache Hit Rate", color: "#10b981", format: (v) => `${(v * 100).toFixed(1)}%` },
  { key: "requests", label: "Request Count", color: "#f59e0b", format: (v) => String(v) },
];
